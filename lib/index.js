...
function dbCleanup(store, interval, KnexStore) {
  return store.ready
    .then(() => {
      ...
      const now = new Date();
      for (let sid in sessionCache) {
        if (!(now <= sessionCache[sid].cacheExpired && now <= sessionCache[sid].expired)) {
          //console.log('connect-session-knex, delete session from cache, sid = %s, session = %s', sid, JSON.stringify(sessionCache[sid], null, 3));
      ...
      return store.knex.transaction((trx) => {
        return trx(store.tablename).del()
...
module.exports = function (connect) {
  ...
  function KnexStore(options = {}) {
    ...
    self.clearInterval = options.clearInterval;
  ...
  KnexStore.prototype.get = function (sid, fn) {
    ...
    return self.ready.then(() => {
      ...
      const now = new Date();
      if (sessionCache[sid] && now <= sessionCache[sid].cacheExpired && now <= sessionCache[sid].expired) {
        const session = sessionCache[sid].sess;
        //console.log('connect-session-knex, got session from cache, sid = %s, session = %s', sid, JSON.stringify(session, null, 3));
        return fn(null, session)
      }

      const condition = expiredCondition(self.knex);
      return self.knex.transaction(function (trx) {
        return trx.select('sess', 'expired')
          .from(self.tablename)
          ...
          .then((response) => {
            ...
            if (response[0]) {
              ...
              if (ret && ret.passport && ret.passport.user) {
                const session = response[0];
                session.cacheExpired = new Date(new Date().getTime() + self.clearInterval);
                session.sess = ret;
                sessionCache[sid] = session;
                //console.log('connect-session-knex, put session in cache, sid = %s, session = %s', sid, JSON.stringify(session, null, 3));
              }
            }
            return ret;
          })
      }).then((session) => {
        //console.log('connect-session-knex, got session from db, sid = %s, session = %s', sid, JSON.stringify(session, null, 3));
        fn(null, session)
        return null;
  ...
  KnexStore.prototype.set = function (sid, sessObject, fn) {
    ...
    if (isOracle(self.knex)) {
      // oracle optimized query
      return self.ready.then(() => resolve(self.knex
        .raw(getOracleFastQuery(self.tablename, self.sidfieldname), [
          sid,
          dbDate,
          sess,
        ]))
